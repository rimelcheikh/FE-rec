...
import numpy as np
import torch
from utils_DACL import calc_metrics
...
# Modules
from utils import umath, load_ck
...
base_path_experiment = "./experiments/ESR/CK/"
...
def main():
    ...
    data_transforms = [transforms.ColorJitter(brightness=0.5,
                       ...
                       transforms.RandomAffine(degrees=30,
                                               translate=(.1, .1),
                                               scale=(1.0, 1.25),
                                               interpolation=InterpolationMode.BILINEAR)] #transforms.Grayscale(num_output_channels=1),
    ...
    for fold in range(0, num_folds):
        ...
        # Set optimizer
        """optimizer = optim.SGD([{'params': net.base.parameters(), 'lr': 0.1, 'momentum': 0.9, 'weight_decay': 1e-4},
                               {'params': net.branches[-1].parameters(), 'lr': 0.1, 'momentum': 0.9, 'weight_decay': 1e-4}])"""
        ...
        val_data = load_ck.CohnKanade(fold, 'validation',
        ...
        val_loader = torch.utils.data.DataLoader(val_data, batch_size = 16, num_workers = 2, shuffle = False, pin_memory = True)
        
        
        test_data = load_ck.CohnKanade(fold, 'testing',
                                            transforms=transforms.Compose([transforms.Grayscale(num_output_channels=1),
                                                                            transforms.Resize(96),]),
                                             base_path_to_dataset='../DB/CK+/')
        print('Testing set size:', test_data.__len__())
        
        test_loader = torch.utils.data.DataLoader(test_data, batch_size = 16, num_workers = 2, shuffle = False, pin_memory = True)
        
        
        train_data = load_ck.CohnKanade(fold, 'training',
                                                 transforms=transforms.Compose(data_transforms),
                                                 base_path_to_dataset='../DB/CK+/')
        print('Training set size:', train_data.__len__())
        
        train_loader = torch.utils.data.DataLoader(train_data, batch_size = 16, num_workers = 2, shuffle = True, pin_memory = True)
        ...
        while True:
            ...
            for epoch in range(max_training_epoch):
                ...
                for inputs, labels, _ in train_loader:
                    ...
                    running_updates += 1
                scheduler.step()
                save_results(y_pred, y_true, y_scores, os.path.join(base_path_experiment, name_experiment, 'Fold-{:02d}'.format(fold + 1), "Branch_"+str(net.get_ensemble_size()), "epoch_"+str(epoch+1)), "train")
                ...
                if ((epoch % validation_interval) == 0) or ((epoch + 1) == max_training_epoch):
                    ...
                    save_results(y_pred_val, y_true_val, y_scores_val, os.path.join(base_path_experiment, name_experiment, 'Fold-{:02d}'.format(fold + 1),"Branch_"+str(net.get_ensemble_size()), "epoch_"+str(epoch+1)), "valid")
...
def test(epoch, test_loader, model, fold, current_branch_on_training=0):
    ...
    save_results(y_pred, y_true, y_scores, os.path.join(base_path_experiment, name_experiment, 'Fold-{:02d}'.format(fold + 1),"Branch_"+str(model.get_ensemble_size()), "epoch_"+str(epoch+1)), "test")
