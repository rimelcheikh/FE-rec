...
def save_results(y_pred, y_true, y_scores, base_path_his, mode):
     ...
     if(not isinstance(y_pred, list)): 
         ...
         for i in range(len(y_pred)):
             if(y_pred[i] != []):
                 y_pred[i] = torch.cat(y_pred[i]).cpu().numpy()
                 y_true[i] = torch.cat(y_true[i]).cpu().numpy()
                 y_scores[i] = torch.cat(y_scores[i]).cpu().numpy()
                 
         y_pred = np.array(y_pred, dtype=object)
         y_true = np.array(y_true, dtype=object)
         y_scores = np.array(y_scores, dtype=object)
